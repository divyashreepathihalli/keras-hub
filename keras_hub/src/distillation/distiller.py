...
from .strategies import AttentionDistillation
from .strategies import FeatureDistillation
from .strategies import LogitsDistillation
...
class Distiller(keras.Model):
    ...
    def __init__(
        self, student, teacher, strategies=None, name="distiller", **kwargs
    ):
        super().__init__(name=name, **kwargs)
        if not isinstance(student, keras.Model):
            raise ValueError(
                f"`student` must be a `keras.Model` instance, got: {student}"
            )
        if not isinstance(teacher, keras.Model):
            raise ValueError(
                f"`teacher` must be a `keras.Model` instance, got: {teacher}"
            )
        ...
        if not isinstance(self.strategies, list):
            raise ValueError(
                f"`strategies` must be a list of strategy instances, got: {self.strategies}"
            )

        self.student_loss_fn = None
        self.distillation_loss_fn = (
            None  # For "classic" distillation if no strategies.
        )
        ...
        )  # Tracks sum of strategy losses or classic distillation loss
        self.total_loss_tracker = keras.metrics.Mean(
            name="total_loss"
        )  # Tracks the final weighted sum loss
    ...
    def compile(
        ...
        **kwargs,
        ...
        self.distillation_loss_fn = (
            distillation_loss_fn  # Used only if self.strategies is empty
        )
        self.alpha = alpha
        self.temperature = temperature

        if not self.strategies and (
            self.student_loss_fn is None and self.distillation_loss_fn is None
        ):
            raise ValueError(
    ...
    def compute_loss(self, x, y, y_pred, sample_weight=None, training=True):
        ...
        if (
            y_pred is not None
            and hasattr(y_pred, "shape")
            and y_pred.shape.rank > 0
        ):  # Check if y_pred is a tensor with shape
            zero_loss_ref = keras.ops.zeros_like(
                y_pred[0]
            )  # Use a slice if y_pred is batched
            if (
                zero_loss_ref.shape.rank > 0
            ):  # If y_pred[0] is still a tensor, sum it to get a scalar
        ...
        if (
            self.student_loss_fn is not None
            and y is not None
            and self.alpha > 0
        ):
            ...
            self.student_loss_tracker.update_state(
                current_student_loss
            )  # Track unweighted student loss
        else:
            self.student_loss_tracker.update_state(
                zero_loss_ref
            )  # Update with 0 if not used or alpha is 0
        ...
        if self.strategies:
            ...
            teacher_outputs_main = self.teacher(
                x, training=False
            )  # Teacher always in inference mode
            ...
            for strategy in self.strategies:
                if isinstance(strategy, LogitsDistillation):
                    ...
                    loss_val = strategy.compute_loss(
                        teacher_outputs_main, y_pred
                    )  # Weight is applied inside strategy.compute_loss
                    current_distillation_loss_total_for_strategies += loss_val
                elif isinstance(
                    strategy, (AttentionDistillation, FeatureDistillation)
                ):
                    # These strategies manage their own submodel calls using `x`
                    loss_val = strategy.compute_loss(
                        self.teacher, self.student, x
                    )  # Weight is applied inside strategy.compute_loss
                    current_distillation_loss_total_for_strategies += loss_val
                else:
            ...
            distillation_loss_sum = (
                current_distillation_loss_total_for_strategies
            )
            self.distillation_loss_tracker.update_state(
                distillation_loss_sum
            )  # Sum of all weighted strategy losses
    ...
    @property
    def metrics(self):
        # Keras 3's base Model.metrics property should automatically gather
        # metrics from `compile()` and any Metric instances that are attributes
        # of the model. We add our custom trackers to this.
        # `super().metrics` returns the metrics from the base class, which
        # includes the compiled metrics.
        return super().metrics

    # `get_metrics_result` and `reset_states` are typically handled by the
    # base Model by virtue of including trackers in `self.metrics`.
    # However, the design doc specifies them, so we can ensure they are
    # explicitly here or confirm if the base Model behavior is sufficient.
    # For now, let's rely on the base Model's handling via the `metrics`
    # property.
    # If custom behavior beyond what `metrics` property provides is needed,
    # we can implement them. The design doc's `reset_states` example is good
    # practice.

    def reset_states(self):
        # Call parent's reset_states for compiled metrics and other internal
        # states. This will reset all metrics in the `self.metrics` property,
        # including our trackers.
        super().reset_states()
