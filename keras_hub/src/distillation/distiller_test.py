...
class DistillerTest(TestCase):
    def _create_simple_model(
        ...
        for i in range(num_hidden_layers):
            x = keras.layers.Dense(
                hidden_dim, activation="relu", name=f"{name}_hidden_{i}"
            )(x)
        x = keras.layers.Dense(
            hidden_dim // 2,
            activation="relu",
            name=f"{name}_intermediate_features",
        )(x)
        outputs = keras.layers.Dense(output_dim, name=f"{name}_output_logits")(
            x
        )
    ...
    def test_distiller_instantiation(self):
        distiller = Distiller(
            student=self.student_model, teacher=self.teacher_model
        )
    ...
    def test_distiller_compile_full_classic(self):
        ...
        self.assertIsNotNone(distiller.distillation_loss_fn)
        self.assertTrue(
            any(m.name == "categorical_accuracy" for m in distiller.metrics)
        )
        self.assertEqual(distiller.alpha, 0.3)
        self.assertEqual(distiller.temperature, 2.5)

    def test_distiller_compile_with_strategies_ignores_classic_distill_loss(
        self,
    ):
        strategies = [
            LogitsDistillation(
    ...
    def test_distiller_compile_value_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "either `student_loss_fn` or `distillation_loss_fn`"
        ):
            distiller.compile(optimizer=keras.optimizers.Adam(), alpha=0.5)

        with self.assertRaisesRegex(
            ValueError, "`student` must be a `keras.Model` instance"
        ):
            Distiller(student="not_a_model", teacher=self.teacher_model)

        with self.assertRaisesRegex(
            ValueError, "`teacher` must be a `keras.Model` instance"
        ):
    ...
    def test_distiller_call_behaves_as_student(self):
        ...
        np.testing.assert_array_almost_equal(
            distiller_output.numpy(), student_output.numpy()
        )

    def _run_train_step_classic_distillation(
        self, use_student_loss, use_classic_distill_loss
    ):
        ...
        student_loss_fn = (
            keras.losses.MeanSquaredError() if use_student_loss else None
        )
        distill_loss_fn = (
            keras.losses.KLDivergence() if use_classic_distill_loss else None
        )
        alpha = 0.0
        if use_student_loss and use_classic_distill_loss:
            alpha = 0.5
        elif use_student_loss:
            alpha = 1.0

        effective_distill_loss_fn = distill_loss_fn
        if (
            student_loss_fn is None and distill_loss_fn is None
        ):  # Ensure compile validity
        ...
        initial_student_weights = [
            w.numpy() for w in self.student_model.trainable_variables
        ]

        data_to_pass = (
            (self.dummy_x, self.dummy_y) if student_loss_fn else (self.dummy_x,)
        )
        if not use_student_loss:  # If student_loss_fn is None, y should be None
            data_to_pass = (self.dummy_x,)

        results = distiller.train_on_batch(x=data_to_pass[0], y=data_to_pass[1] if len(data_to_pass) > 1 else None, return_dict=True)

        self.assertIn("loss", results)
        self.assertIsInstance(results["loss"], (float, np.floating, np.ndarray))
        self.assertIn("student_loss", results)
        self.assertIsInstance(
            results["student_loss"], (float, np.floating, np.ndarray)
        )
        self.assertIn("distillation_loss", results)
        self.assertIsInstance(
            results["distillation_loss"], (float, np.floating, np.ndarray)
        )

        if use_student_loss and student_loss_fn:
            self.assertTrue(
                results["student_loss"] >= 0
            )  # Can be >0 if y provided
        else:
            self.assertEqual(results["student_loss"], 0.0)

        if use_classic_distill_loss:
            self.assertTrue(
                results["distillation_loss"] >= 0
            )  # KLD is >=0

        updated_student_weights = [
            w.numpy() for w in self.student_model.trainable_variables
        ]
        weight_changed = any(
            not np.array_equal(initial, updated)
            for initial, updated in zip(
                initial_student_weights, updated_student_weights
            )
        )

        a_loss_was_active = (
            use_student_loss and student_loss_fn and alpha > 0
        ) or (use_classic_distill_loss and distill_loss_fn and (1 - alpha) > 0)

        if a_loss_was_active and self.student_model.trainable_variables:
            self.assertTrue(
                weight_changed,
                "Student weights were not updated when a loss was active.",
            )
        elif (
            self.student_model.trainable_variables
        ):  # No loss active but trainable vars exist
            self.assertFalse(
                weight_changed,
                "Student weights changed unexpectedly with no active loss.",
            )

        if student_loss_fn and distiller.compiled_metrics is not None:
            self.assertIn("mean_absolute_error", results)
            self.assertTrue(results["mean_absolute_error"] >= 0)

    def test_train_step_classic_student_and_distill_loss(self):
        self._run_train_step_classic_distillation(
            use_student_loss=True, use_classic_distill_loss=True
        )

    def test_train_step_classic_student_only_loss(self):
        self._run_train_step_classic_distillation(
            use_student_loss=True, use_classic_distill_loss=False
        )

    def test_train_step_classic_distill_only_loss(self):
        self._run_train_step_classic_distillation(
            use_student_loss=False, use_classic_distill_loss=True
        )
    ...
    def test_test_step_classic_distillation(self):
        ...
        results = distiller.test_on_batch(x=self.dummy_x, y=self.dummy_y, return_dict=True)
        ...
        self.assertFalse(
            weight_changed, "Student weights were updated during test_step."
        )
    ...
    def test_train_step_with_no_labels(self):
        ...
        results = distiller.train_on_batch(x=self.dummy_x, return_dict=True)
        self.assertIn("loss", results)
        self.assertEqual(results["student_loss"], 0.0)
        self.assertTrue(results["distillation_loss"] >= 0.0)
    ...
    def test_reset_states(self):
        ...
        distiller.train_on_batch(x=self.dummy_x, y=self.dummy_y, return_dict=True)
        self.assertNotEqual(distiller.total_loss_tracker.result().numpy(), 0.0)
        self.assertNotEqual(
            distiller.student_loss_tracker.result().numpy(), 0.0
        )
        ...
        self.assertEqual(
            distiller.distillation_loss_tracker.result().numpy(), 0.0
        )
        if distiller.compiled_metrics and distiller.metrics:
            self.assertEqual(
                distiller.metrics[0].result().numpy(), 0.0
            )
    ...
    def test_logits_distillation_strategy_train_step(self):
        ...
        results = distiller.train_on_batch(x=self.dummy_x, y=self.dummy_y, return_dict=True)
        self.assertIn("loss", results)
        if distiller.alpha > 0:
            self.assertTrue(results["student_loss"] >= 0)
        else:
            self.assertEqual(results["student_loss"], 0.0)
        self.assertTrue(results["distillation_loss"] >= 0)
    ...
    def test_attention_distillation_strategy_train_step(self):
        ...
        student_model_att_compat = self._create_simple_model(name="student_att_compat", hidden_dim=32) # intermediate = 16
        ...
        attention_strategy = AttentionDistillation(
            teacher_layer=teacher_att_layer,
            student_layer=student_att_layer,
            loss_fn=keras.losses.MeanSquaredError(),
            weight=0.7,
        )
        distiller = Distiller(
            student=student_model_att_compat,
            teacher=self.teacher_model,
            strategies=[attention_strategy],
        )
        distiller.compile(
            optimizer=keras.optimizers.Adam(),
            student_loss_fn=keras.losses.MeanSquaredError(),
        ...
        _ = distiller.train_on_batch(x=self.dummy_x, y=self.dummy_y, return_dict=True)  # Build submodels
        results = distiller.train_on_batch(x=self.dummy_x, y=self.dummy_y, return_dict=True)
    ...
    def test_feature_distillation_strategy_train_step_no_projection(self):
        student_model_compat = self._create_simple_model(
            name="student_compat", hidden_dim=32
        )  # intermediate = 16
        ...
        feature_strategy = FeatureDistillation(
            ...
            student_layer="student_compat_intermediate_features",
            loss_fn=keras.losses.CosineSimilarity(),
            weight=0.6,
            projection=False,
        )
        distiller = Distiller(
            student=student_model_compat,
            teacher=self.teacher_model,
            strategies=[feature_strategy],
        )
        distiller.compile(optimizer=keras.optimizers.Adam(), alpha=0.0)
        results = distiller.train_on_batch(x=self.dummy_x, return_dict=True)
        self.assertIn("loss", results)
        self.assertNotEqual(results["distillation_loss"], 0)
    ...
    def test_feature_distillation_with_projection(self):
        ...
        feature_strategy = FeatureDistillation(
            teacher_layer=teacher_feat_layer,
            student_layer=student_feat_layer,
            loss_fn=keras.losses.MeanSquaredError(),
            weight=0.6,
            projection=True,
        )
        ...
        distiller.compile(optimizer=keras.optimizers.Adam(), alpha=0.0)
        _ = distiller.train_on_batch(x=self.dummy_x, return_dict=True)

        self.assertIsNotNone(feature_strategy.projection_layer)
        self.assertTrue(feature_strategy.projection_layer.built)
        expected_student_dim = self.student_model.get_layer(
            student_feat_layer
        ).output.shape[-1]
        self.assertEqual(
            feature_strategy.projection_layer.units, expected_student_dim
        )
        results = distiller.train_on_batch(x=self.dummy_x, return_dict=True)
        self.assertIn("loss", results)
        self.assertTrue(results["distillation_loss"] > 0)
    ...
    def test_multiple_strategies_train_step(self):
        student_model_multi = self._create_simple_model(
            name="student_multi", hidden_dim=32
        )
        _ = student_model_multi(self.dummy_x)  # Build

        logits_strat = LogitsDistillation(
            temperature=2.0, loss_fn=keras.losses.KLDivergence(), weight=0.5
        )
        feature_strat = FeatureDistillation(
            teacher_layer="teacher_intermediate_features",
            student_layer="student_multi_intermediate_features",
            loss_fn=keras.losses.MeanSquaredError(),
            weight=0.3,
            projection=False,
        )
        distiller = Distiller(
            student=student_model_multi,
            teacher=self.teacher_model,
            strategies=[logits_strat, feature_strat],
        )
        distiller.compile(
            optimizer=keras.optimizers.Adam(),
            student_loss_fn=keras.losses.MeanSquaredError(),
    ...
    def _check_layer_not_found(
        self, strategy_class, bad_teacher_layer, bad_student_layer, match_regex
    ):
        if bad_teacher_layer:
            strategy = strategy_class(
                "nonexistent_teacher_layer",
            ...
            strategy = strategy_class(
                ...
                "nonexistent_student_layer",
                keras.losses.MeanSquaredError(),
                1.0,
            )

        distiller = Distiller(
            self.student_model, self.teacher_model, strategies=[strategy]
        )
        distiller.compile(optimizer=keras.optimizers.Adam())
        with self.assertRaisesRegex(ValueError, match_regex):
            distiller.train_on_batch(x=self.dummy_x, return_dict=True)

    def test_attention_distillation_layer_not_found(self):
        self._check_layer_not_found(
            AttentionDistillation,
            True,
            False,
            "Teacher layer 'nonexistent_teacher_layer' not found",
        )
        self._check_layer_not_found(
            AttentionDistillation,
            False,
            True,
            "Student layer 'nonexistent_student_layer' not found",
        )
    ...
    def test_feature_distillation_projection_dim_inference_works_for_lambda(
        self,
    ):
        ...
        lambda_output_dim = teacher_intermediate_layer.output.shape[-1] // 2
        intermediate_out = keras.layers.Lambda(
            lambda t: t[:, :lambda_output_dim],
            output_shape=(lambda_output_dim,),
            name="student_intermediate_lambda",
        )(student_input)
        output_logits = keras.layers.Dense(
            5, name="student_output_logits_lambda"
        )(intermediate_out)
        student_model_lambda = keras.Model(
            inputs=student_input, outputs=output_logits, name="student_lambda"
        )
        ...
        try:
            ...
            expected_units = student_model_lambda.get_layer(
                "student_intermediate_lambda"
            ).output_shape[-1]
            self.assertEqual(
                feature_strategy.projection_layer.units, expected_units
            )
        except ValueError as e:
            self.fail(
                f"FeatureDistillation with projection failed to infer dim for Lambda: {e}"
            )
    ...
    def test_feature_distillation_projection_dim_inference_fail_mocked(self):
        ...
        student_model_mock = self._create_simple_model(
            name="student_mock_shape"
        )
        ...
        class MockLayerWithNoneShapeDim(keras.layers.Layer):
            def __init__(self, original_layer, **kwargs):
                super().__init__(
                    name=original_layer.name, dtype=original_layer.dtype
                )
                self._original_layer_instance = original_layer
                self._dynamic_output_shape = list(original_layer.output.shape)
                self._dynamic_output_shape[-1] = None

            @property
            def output_shape(self):
                return tuple(self._dynamic_output_shape)

            def call(self, inputs, *args, **kwargs):
                return self._original_layer_instance(inputs, *args, **kwargs)

            def build(self, input_shape):
                if (
                    hasattr(self._original_layer_instance, "build")
                    and not self._original_layer_instance.built
                ):
        ...
        distiller_fail = Distiller(
            student=student_model_mock,
            teacher=self.teacher_model,
            strategies=[feature_strategy_fail],
        )
        distiller_fail.compile(optimizer=keras.optimizers.Adam())

        with self.assertRaisesRegex(
            ValueError,
            "Cannot automatically infer `student_output_dim` for projection",
        ):
            distiller_fail.train_on_batch(x=self.dummy_x, return_dict=True)
...
if __name__ == "__main__":
    ...
    is_standard_unittest_case = (
        TestCase.__module__ == "unittest.case"
        and TestCase.__name__ == "TestCase"
    )
