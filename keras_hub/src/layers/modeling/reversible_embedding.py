    ...
    def quantize(self, mode, type_check=True):
        ...
        if mode == "int8":
            embeddings, embeddings_scale = abs_max_quantize(
                self.embeddings, axis=-1  # Use float self.embeddings
            )
            embeddings_scale = ops.squeeze(embeddings_scale, axis=-1)
            del self.embeddings  # Delete float self.embeddings
            if not self.tie_weights:
        ...
        if mode == "int8":
            self.embeddings.assign(embeddings)  # Assign to int8 self.embeddings
