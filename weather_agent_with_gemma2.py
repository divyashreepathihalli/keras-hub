#!/usr/bin/env python3
"""
Weather Agent with KerasHub Gemma2 Integration

This example demonstrates a practical weather agent that uses KerasHub Gemma2 models
with Google's ADK to provide weather information, analysis, and recommendations.
"""
...
async def test_weather_agent():
    ...
    print("=== Weather Agent with KerasHub Gemma2 Test ===\n")
    ...
    for action, params in actions:
        ...
        result = await weather_tool.call(None, action=action, **params)
        print(f"Result: {result.get('result', 'No result')[:200]}...")
    
    print("\n=== Weather Agent Test Complete ===")
...
async def interactive_weather_session():
    """Run an interactive weather agent session."""
    
    print("\n=== Interactive Weather Agent Session ===")
    ...
    while True:
        try:
            print("\nAvailable actions:")
            print("1. analyze <weather_data>")
            print("2. report <location> <conditions>")
            print("3. advice <activity> <weather_data>")
            print("4. quit")
            ...
            if len(parts) < 2:
                print("Please provide an action and parameters")
                continue
...
def main():
    """Main function to run the weather agent tests."""
    
    print("Starting Weather Agent with KerasHub Gemma2...")
    ...
    try:
        asyncio.run(interactive_weather_session())
    except KeyboardInterrupt:
